...
private struct MemoCellView: View {
    @EnvironmentObject private var pathModel: PathModel
    @EnvironmentObject private var memoListViewModel: MemoListViewModel
    @State private var isRemoceSelected: Bool
    private var memo: MemoModel
    
    fileprivate init(
        isRemoceSelected: Bool = false,
        memo: MemoModel
    ) {
        _isRemoceSelected = State(initialValue: isRemoceSelected)
        self.memo = memo
    }
                ...
                VStack(spacing: 10) {
                    HStack {
                        ...
                        if memoListViewModel.isEditMemoMode {
                            Button(
                                action: {
                                    isRemoceSelected.toggle()
                                    memoListViewModel.memoRemoveSelectedBoxTapped(memo)
                                },
                                label: { isRemoceSelected ? Image("selectedBox") : Image("unSelectedBox") }
                            )
                        }
                    }
